...
This is the simplest part. I get new torrents on to my Mac Mini several different ways; I keep Transmission running on the Mac Mini monitoring my Downloads folder for new torrent files. I also run the TVShows daemon to assist with downloading of torrents for new episodes. On my primary machine (my MacBook), I run Hazel:"http://www.noodlesoft.com/hazel.php", having it monitor my Macbook's downloads folder for any torrents, which is transfers over to the Mac Mini Downloads folder for processing automatically.

All completed downloads are moved into a "New" folder by Transmission, ready to be organised.

h2. Processing DVD rips

Running RipIt, I can continue to replace DVDs and they are automatically ripped as *.dvdmedia files. Once they are ripped, they need to be processed and individual tracks encoded using a combination of HandBrakeCLI, my custom ripper script and the RipperHUD Sinatra app. This process is explained in more detail below. Encoded movies are moved to the "New" folder, ready to be organised.

h2. Processing "New" movies

The media-mover script is configured using launchd to monitor new files in the "New" folder. If the file looks like a TV show (e.g. "Some.Show.SXXEXX.avi"), media-mover will create folders for the show name and season in a specified location and move the file there. It will optionally inform the Plex media server of the new file. Example:
...
These will be moved to a specified folder (this should be set to the "new" folder) where they are ready to be organised.
...
http://skitch.com/lukeredpath/bkqwq/picture-2
